...
export function fetchRenewals(borrowId: number | string) {
  return get<BorrowRenewal[]>(API_PATHS.borrowRenewals(borrowId))
}

export function applyRenewal(borrowId: number | string, payload: CreateRenewalPayload) {
  return post<BorrowRenewal>(API_PATHS.borrowRenewals(borrowId), payload)
}
...
export function rejectRenewal(id: number | string, reason?: string) {
  return post<null>(API_PATHS.renewalReject(id), reason ? { reason } : {})
}
